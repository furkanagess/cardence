...
| Healthcheck 14x "service unavailable" | `AllowedHosts` içinde `healthcheck.railway.app` olmalı |
| `Invalid Hostname` on `*.up.railway.app` | `AllowedHosts` içine `*.up.railway.app` ekle (Railway generated domain) |
